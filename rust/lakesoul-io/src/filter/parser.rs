...
impl Parser {
    ...
    pub(crate) fn parse_proto(plan: &Plan) -> Result<Arc<Expr>> {
    ...
    fn parse_rel(rel: &Rel, extensions: &HashMap<u32, &String>) -> Result<Arc<Expr>> {
        match &rel.rel_type {
    ...
    // recursion
    fn parse_rex(e: &Expression, input_schema: &DFSchema, extensions: &HashMap<u32, &String>) -> Result<Arc<Expr>> {
        match &e.rex_type {
            ...
            Some(RexType::ScalarFunction(f)) => {
                ...
                match fn_type {
                    ScalarFunctionType::Builtin(fun) => {
                        ...
                        Ok(Arc::new(Expr::ScalarFunction(expr::ScalarFunction { fun, args })))
                    }
                        ...
                        match (lhs, rhs) {
                            (Some(ArgType::Value(l)), Some(ArgType::Value(r))) => {
                                Ok(Arc::new(Expr::BinaryExpr(BinaryExpr {
                                    left: Box::new(Parser::parse_rex(l, input_schema, extensions)?.as_ref().clone()),
                                    op,
                                    right: Box::new(Parser::parse_rex(r, input_schema, extensions)?.as_ref().clone()),
                                })))
                            }
                    ...
                    ScalarFunctionType::Not => {
                        ...
                        match &arg.arg_type {
                            Some(ArgType::Value(e)) => {
                                let expr = Parser::parse_rex(e, input_schema, extensions)?.as_ref().clone();
...
fn from_substrait_type(dt: &substrait::proto::Type) -> Result<(DataType, Nullability)> {
    match &dt.kind {
        Some(s_kind) => match s_kind {
            ...
            r#type::Kind::Timestamp(ts) => match ts.type_variation_reference {
                TIMESTAMP_SECOND_TYPE_REF => Ok((DataType::Timestamp(TimeUnit::Second, None), ts.nullability())),
                TIMESTAMP_MILLI_TYPE_REF => Ok((DataType::Timestamp(TimeUnit::Millisecond, None), ts.nullability())),
                TIMESTAMP_MICRO_TYPE_REF => Ok((DataType::Timestamp(TimeUnit::Microsecond, None), ts.nullability())),

...
use spark_murmur3::spark_murmur3_32_for_bytes;

mod spark_murmur3;

pub const HASH_SEED: u32 = 42;

fn hash_null(hashes_buffer: &'_ mut [u32], mul_col: bool) {
    if mul_col {
...
/// If `rehash==true` this combines the previous hash value in the buffer
/// with the new hash using `combine_hashes`
fn hash_array<T>(array: T, hashes_buffer: &mut [u32], rehash: bool)
where
    T: ArrayAccessor,
    T::Item: HashValue,
{
...
mod tests {
    ...
    fn create_hashes_for_decimal_array() -> Result<()> {
        ...
        let hashes = create_hashes(&[array_ref], hashes_buff)?;
    ...
    fn create_hashes_for_float_arrays() -> Result<()> {
        ...
        let hashes = create_hashes(&[f32_arr], hashes_buff)?;
        assert_eq!(hashes.len(), 4,);

        let hashes = create_hashes(&[f64_arr], hashes_buff)?;
        assert_eq!(hashes.len(), 4,);
    ...
    fn create_hashes_binary() -> Result<()> {
        ...
        let hashes = create_hashes(&[byte_array], hashes_buff)?;
    ...
    fn create_hashes_fixed_size_binary() -> Result<()> {
        ...
        let hashes = create_hashes(&[fixed_size_binary_array], hashes_buff)?;
        assert_eq!(hashes.len(), 3,);
    ...
    fn create_hashes_for_dict_arrays() {
        ...
        create_hashes(&[dict_array], &mut dict_hashes).unwrap();
    ...
    fn create_multi_column_hash_for_dict_arrays() {
        ...
        create_hashes(&[dict_array.clone()], &mut one_col_hashes).unwrap();

        let mut two_col_hashes = vec![0; strings1.len()];
        create_hashes(&[dict_array, string_array], &mut two_col_hashes).unwrap();

...
use crate::{config::IOConfig, writer::async_writer::InMemBuf};
...
pub struct MultiPartAsyncWriter {
    ...
    arrow_writer: Box<dyn RecordBatchWriter>,
...
impl MultiPartAsyncWriter {
    pub async fn try_new_with_context(
        ...
        task_context: Arc<TaskContext>,
        writer: Box<dyn RecordBatchWriter>,
    ) -> Result<Self, Report> {
        todo!()
    ...
    async fn write_batch(
        batch: RecordBatch,
        arrow_writer: &mut Box<dyn RecordBatchWriter>,
    ...
    pub async fn write_part(
        ...
        let other = in_mem_buf.split().freeze();
        writer.put(other);
        Ok(())
    }

...
LakeSoulDataReader::LakeSoulDataReader(
...
int LakeSoulDataReader::GetThreadNum() const { return thread_num_; }
...
lakesoul::IOConfig *LakeSoulDataReader::CreateIOConfig() {
  ...
  for (const auto &field : schema_->fields()) {
    if (std::find_if(
            partition_info_.begin(), partition_info_.end(),
            [&](const std::pair<std::string, std::string> &partition_pair) {
              return partition_pair.first == field->name();
            }) != partition_info_.end()) {
  ...
  if (partition_fields.size() > 0 && retain_partition_columns_) {
    ...
    builder = lakesoul::lakesoul_config_builder_set_partition_schema(
        builder, reinterpret_cast<lakesoul::c_ptrdiff_t>(&c_schema));
  }

  for (auto &&[key, value] : partition_info_) {
    builder = lakesoul::lakesoul_config_builder_set_default_column_value(
        builder, key.c_str(), value.c_str());
  }
      ...
      +[](bool status, const char *err, const void *data) {
        ...
        if (!status) {
          ...
          if (err != nullptr)
            sout << ": " << err;
          std::string message = sout.str();
          closure->future.MarkFinished(
              arrow::Status::IOError(std::move(message)));
        } else {
...
bool LakeSoulDataReader::IsFinished() const { return finished_; }

arrow::Future<std::shared_ptr<arrow::RecordBatch>>
LakeSoulDataReader::ReadRecordBatchAsync() {
  struct Closure {
    std::shared_ptr<LakeSoulDataReader> reader;
  ...
  lakesoul::next_record_batch_with_data(
      ...
      data, +[](int32_t n, const char *err, const void *data) {
        ...
        if (n < 0) {
          ...
          auto bat = arrow::ImportRecordBatch(&closure->c_arrow_array,
                                              &closure->c_arrow_schema)
                         .ValueOrDie();
...
void LakeSoulDataReader::SetObjectStoreConfigs(

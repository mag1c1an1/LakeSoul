...
#include "lakesoul/dataset.h"
#include "lakesoul/fragment.h"
...
LakeSoulDataset::LakeSoulDataset(
...
LakeSoulDataset::ReplaceSchema(std::shared_ptr<arrow::Schema> schema) const {
  ...
  arrow::Result<std::shared_ptr<arrow::dataset::Dataset>> result(
      std::move(dataset));
  return result;
}
...
LakeSoulDataset::GetFragmentsImpl(arrow::compute::Expression predicate) {
  try {
    ...
    arrow::Result<arrow::dataset::FragmentIterator> result(
        arrow::MakeVectorIterator(std::move(fragments)));
    return result;
...
arrow::Result<arrow::dataset::FragmentGenerator>
LakeSoulDataset::GetFragmentsAsyncImpl(arrow::compute::Expression predicate,
                                       arrow::internal::Executor *executor) {
  // TODO
  return arrow::Status::NotImplemented("async is not implemented");
}
...
void LakeSoulDataset::AddPartitionKeyValue(const std::string &key,
                                           const std::string &value) {
  partition_info_.push_back(std::make_pair(key, value));
}

void LakeSoulDataset::AddPartitionKeyValues(
    const std::vector<std::pair<std::string, std::string>> &key_values) {
  partition_info_.insert(partition_info_.end(), key_values.begin(),
                         key_values.end());
}

int LakeSoulDataset::GetBatchSize() const { return batch_size_; }

void LakeSoulDataset::SetBatchSize(int batch_size) {
  batch_size_ = batch_size >= 1 ? batch_size : 1;
}

int LakeSoulDataset::GetThreadNum() const { return thread_num_; }
...
void LakeSoulDataset::SetObjectStoreConfig(const std::string &key,
                                           const std::string &value) {
